...
DUQUE_c=pd.read_feather(r'Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\data_Coherence_column_norm_DUQUE.feather')
#BIO_c['group'].replace({'G1':'Control','G2':'Control','CTR':'Control'}, inplace=True)

#Entropy
DUQUE_e=pd.read_feather(r'Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\data_Entropy_column_norm_DUQUE.feather')
...
#data_roi=data_roi.drop(columns=['level_0','index'])#va este
...
entropy=[ 'Entropy_Delta','Entropy_Theta', 'Entropy_Alpha-1', 'Entropy_Alpha-2', 'Entropy_Beta1',
       'Entropy_Beta2', 'Entropy_Beta3', 'Entropy_Gamma']
...
d_sl=d_sl[d_sl['database']=='DUQUE']
d_sl.reset_index().to_feather('Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\Datos_para_graficos_SL_desdeunionpotencias_roi.feather')
...
d_c=d_c[d_c['database']=='DUQUE']
d_c.reset_index().to_feather('Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\Datos_para_graficos_COHERENCE_desdeunionpotencias_roi.feather')

d_e=pd.DataFrame(columns=['participant_id', 'visit', 'group', 'condition', 'database', 'age','sex', 'education', 'MM_total', 'FAS_F', 'FAS_A', 'FAS_S', 'Entropy', 'Band'])
...
for i in entropy:
    ...
    d_sep['Band']=[band]*len(d_sep)
    #d_sep['ROI']=[r]*len(d_sep)
    d_sep= d_sep.rename(columns={i:'Entropy'})
    d_e=d_e.append(d_sep,ignore_index = True) #Uno el dataframe 
d_e=d_e[d_e['database']=='DUQUE']
d_e.reset_index().to_feather('Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\Datos_para_graficos_ENTROPY_desdeunionpotencias_roi.feather')
...
d_sl['ROI']=d_sl['ROI'].replace({'ROI_':''}, regex=True)#Quito el _ y lo reemplazo con '' 
d_sl=d_sl[d_sl['database']=='DUQUE']
d_sl.reset_index().to_feather('Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\Datos_para_graficos_SL_desdeunionpotencias_Componentes.feather')
...
for i in coherence:
    ...
    d_c=d_c.append(d_sep,ignore_index = True) #Uno el dataframe 
d_c=d_c[d_c['database']=='DUQUE']
d_c.reset_index().to_feather('Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\Datos_para_graficos_COHERENCE_desdeunionpotencias_componentes.feather')
...
for i in entropy:
    ...
    d_e=d_e.append(d_sep,ignore_index = True) #Uno el dataframe 
d_e=d_e[d_e['database']=='DUQUE']
d_e.reset_index().to_feather('Manipulacion- Rois-Componentes de todas las DB\Datosparaorganizardataframes\Datos_para_graficos_ENTROPY_desdeunionpotencias_componentes.feather')
