...
def get_dataframe_columnsROI_Coherencia(THE_DATASET): 
    ...
    for i in range(len(eegs_powers)):
        ...
        for b,band in enumerate(bandas):
            ...
            c_promedio_roi = np.average(np.array(data['coherence'][band])[1])
...
def get_dataframe_columns_cross(THE_DATASET): 
    ...
    layout.get(scope='derivatives', return_type='file')
    eegs_powers= layout.get(extension='.txt', task=task,suffix='norm', return_type='filename')
    eegs_powers = [x for x in eegs_powers if f'desc-channel[{runlabel}]_cross_frequency' in x]
    #eegs_powers = [x for x in eegs_powers if 'coherence_band' in x]
    ...
    for i in range(len(eegs_powers)):
        ...
        with open(eegs_powers[i], 'r') as f:
            data = json.load(f)
        total_channels=np.array(data['channels'])
        bandas = data['cross_frequency']['bands']
        new_rois = []
        potencias_roi_banda=[]

        for roi in rois:
            channels = set(data['cross_frequency']['channels']).intersection(roi)
            new_roi = []
            for channel in channels:
                index=data['cross_frequency']['channels'].index(channel)
                new_roi.append(index)
            new_rois.append(new_roi)
