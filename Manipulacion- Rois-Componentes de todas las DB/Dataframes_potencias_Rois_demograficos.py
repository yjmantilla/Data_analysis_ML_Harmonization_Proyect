...
BIO=pd.read_feather(r'{path}\Datosparaorganizardataframes\data_powers_column_ROI_norm_BIOMARCADORES.feather'.format(path=path))
DUQUE=pd.read_feather(r'{path}\Datosparaorganizardataframes\data_powers_column_ROI_norm_DUQUE.feather'.format(path=path))

datos=pd.concat([SRM,BIO,CHBMP,DUQUE]) #concatenation of data

"Some columns are renamed and their contents are also modified"
datos= datos.rename(columns={'subject':'participant_id','Study':'database'})
