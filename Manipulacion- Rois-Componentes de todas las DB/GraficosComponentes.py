...
data_Comp1.drop(data_Comp1[data_Comp1['group']=='DCL'].index.tolist(),inplace=True)
max=data_Comp1.loc[data_Comp1['Component']=='C23'].loc[data_Comp1['Band']=='Beta3']['Power'].max()
sns.set(rc={'figure.figsize':(15,12)})
sns.set_theme(style="white")
axs=sns.catplot(x='group',y="Power",data=data_Comp1.loc[data_Comp1['Component']=='C23'].loc[data_Comp1['Band']=='Theta'],hue='database',dodge=True, kind="box",palette='winter_r',fliersize=1.5,linewidth=0.5,legend=False)
#plt.yticks(np.arange(0,round(max),0.1))
axs.set(xlabel=None)
axs.set(ylabel=None)
axs.fig.suptitle('Potencia relativa en la banda Theta en la componente C23')
axs.add_legend(loc='upper right',bbox_to_anchor=(0.8,0.94),ncol=4,title="Database")

axs.fig.subplots_adjust(top=0.85,bottom=0.121, right=0.986,left=0.08, hspace=0.138, wspace=0.062) # adjust the Figure in rp
axs.fig.text(0.5, 0.04, 'Grupo', ha='center', va='center')
axs.fig.text(0.01, 0.5,  'Potencia relativa', ha='center', va='center',rotation='vertical')
plt.savefig('grafico poster')
