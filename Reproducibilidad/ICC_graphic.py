...
def barplot_icc_nB_1G(icc_data,x_value,group,plot=False,save=False):
    ...
    if save==True:
        plt.savefig('Reproducibilidad\ICC_Graphics\ICC_{name_group}_{tipo}.png'.format(name_group=group,tipo=x_value))
        plt.close()
    if plot:
        plt.show()
...
def barplot_icc_comp_nG(icc_data,x_value,title,plot=False,save=False):
    ...
    ax.fig.text(0.03, 0.5,  'ICC', ha='center', va='center',rotation='vertical')
    if save==True:
        plt.savefig('Reproducibilidad\ICC_Graphics\ICC_Comparacióngrupos_{tipo}.png'.format(tipo=x_value))
        plt.close()
    if plot:
        plt.show()
...
def barplot_icc_bandsx(icc_data,x_value, title,plot=False,save=False):
    ...
    ax.fig.text(0.03, 0.5,  'ICC', ha='center', va='center',rotation='vertical')
    if save==True:
        plt.savefig('Reproducibilidad\ICC_Graphics\ICC_Comparacióngrupos_bandasx_{tipo}.png'.format(tipo=x_value))
        plt.close()
    if plot:
        plt.show()
